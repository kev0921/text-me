...
const Layout = async ({ children }: LayoutProps) => {
  ...
  const unseenRequestCount = (
    (await fetchRedis(
      ...
      `user:${session.user.id}:incoming_friend_requests`  // value we want to retrieve from the database (incoming friend requests for this current session user)
    )) as User[]
  ).length    // finds number of friend requests
        ...
        <nav className='flex flex-1 flex-col'>
          <ul role='list' className='flex flex-1 flex-col gap-y-7'>
            ...
            <li className='-mx-6 mt-auto flex items-center'>
              ...
              <SignOutButton className='h-full aspect-square bg-gray-400' />
            </li>
          </ul>
        </nav>
      </div>
